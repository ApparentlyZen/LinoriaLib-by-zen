...
function Library:CreateWindow(...)
    ...
    if Config.AutoShow then 
        if Config.AnimateIntro then
            task.spawn(function()
                -- Augmentation du flou pour le focus
                if Library.Blur then
                    Library.Blur.Enabled = true
                    TweenService:Create(Library.Blur, TweenInfo.new(1), { Size = 24 }):Play()
                end

                -- Création du cadre principal (Grand Carré Cyber)
                ...
                -- Double contour avec dégradé néon
                local Stroke = Library:Create('UIStroke', { Color = Library.AccentColor, Thickness = 2, Transparency = 1, Parent = InjectFrame })
                local Glow = Library:Create('UIStroke', { Color = Library.AccentColor, Thickness = 6, Transparency = 1, Parent = InjectFrame })
                
                Library:Create('UIGradient', {
                    Color = ColorSequence.new(Library.AccentColor, Color3.fromRGB(200, 100, 255)),
                    Rotation = 45,
                    Parent = Stroke
                })

                -- Élément décoratif : Carré de scan rotatif
                local ScanFrame = Library:Create('Frame', {
                ...
                local ScanStroke = Library:Create('UIStroke', { Color = Library.AccentColor, Thickness = 1, Transparency = 1, Parent = ScanFrame })
                task.spawn(function()
                    while InjectFrame.Parent do
                        ScanFrame.Rotation = ScanFrame.Rotation + 1
                        task.wait()
                    end
                end)
                ...
                Library:Create('UIGradient', { Color = ColorSequence.new(Library.AccentColor, Color3.new(1,1,1)), Parent = BarFill })
                ...
                TweenService:Create(Glow, IntroFade, { Transparency = 0.6 }):Play()
                TweenService:Create(ScanStroke, IntroFade, { Transparency = 0.8 }):Play()
                TweenService:Create(Title, IntroFade, { TextTransparency = 0 }):Play()
                TweenService:Create(Status, IntroFade, { TextTransparency = 0 }):Play()
                ...
                local Steps = {
                    { 0.2, "ANALYZING SYSTEM..." },
                    { 0.5, "BYPASSING KERNEL..." },
                    { 0.8, "INJECTING MODULES..." },
                    { 1.0, "SUCCESSFUL" }
                }

                for i, step in next, Steps do
                    Status.Text = step[2]
                    if i == #Steps then Status.TextColor3 = Color3.fromRGB(180, 120, 255) end
                    
                    local Progress = TweenService:Create(BarFill, TweenInfo.new(0.7, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), { Size = UDim2.fromScale(step[1], 1) })
                    Progress:Play()
                ...
                -- Disparition
                local OutInfo = TweenInfo.new(0.6, Enum.EasingStyle.Quart, Enum.EasingDirection.In)
                TweenService:Create(InjectFrame, OutInfo, { BackgroundTransparency = 1, Size = UDim2.fromOffset(450, 450) }):Play()
                TweenService:Create(Stroke, OutInfo, { Transparency = 1 }):Play()
                TweenService:Create(Glow, OutInfo, { Transparency = 1 }):Play()
                TweenService:Create(ScanStroke, OutInfo, { Transparency = 1 }):Play()
                TweenService:Create(Title, OutInfo, { TextTransparency = 1 }):Play()
                TweenService:Create(Status, OutInfo, { TextTransparency = 1 }):Play()
                TweenService:Create(BarBack, OutInfo, { BackgroundTransparency = 1 }):Play()
                local FinalTween = TweenService:Create(BarFill, OutInfo, { BackgroundTransparency = 1 })
                FinalTween:Play()
